...
只需要用户在浏览器中中输入http://localhost:12345/client/entrance
...
class EntranceHandler(BaseHandler):
    def GET(self):
        ...
        req_params = {'redirect_uri': 'http://localhost:%s/client/auth_pass' % config.port,
                       'scope': scope,
                       'response_type': 'code',
                       'appid': appid,
                       'userid': userid}
        self.redirect('http://localhost:%s/oauth2/auth?%s' % (config.port, urllib.urlencode(req_params)))
...
class  AuthPassHandler(BaseHandler):
    ...
    def GET(self):
        ...
        url = 'http://localhost:%s/oauth2/access_token?%s' % (config.port, urllib.urlencode(req_params))
        cli = AsyncHTTPClient()
        ...
        if rsp_dict['scope'] == 'snsapi_base':
            ...
            req_params = {'access_token': rsp_dict['access_token'],
                          ...
                          'lang': 'zh_CN'}
            url = 'http://localhost:%s/oauth2/user_info?%s' % (config.port, urllib.urlencode(req_params))
            cli = AsyncHTTPClient()
            response = yield cli.fetch(url) 
